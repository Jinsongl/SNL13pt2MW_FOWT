...
changeline(ADipt,10,['"',ADwnd,'"']   ,nLines_ADipt);                           % WindFile    - Name of file containing wind data (quoted string)
%%
% ------------TurbSim input file-----------
...
%%
%------------HydroDyn input File-----------
WaveMod = 2;    % Incident wave kinematics model {0: none=still water, 
...
% for i=1:size(WaveHs,2)
    ...
    % TurbSim input file
%
changeline(TSinp,4,num2str(WindSeed(1,1), '% 10.0f'),nLines_TSinp);   % 1st random windseed
changeline(TSinp,5,num2str(WindSeed(1,2), '% 10.0f'),nLines_TSinp);   % 1st random windseed

% changeWindLine (Windipt,17,num2str(WindVel(1),'% 10.2f'),nLines_Wnd);

changeline(TSinp,37,num2str(WindVel(1), '% 10.2f'),nLines_TSinp);         % WindVel
...
changeline(HDipt,23,num2str(WaveSeed(1,1), '% 10.0f'),nLines_HDipt);% 1st random waveseed
changeline(HDipt,24,num2str(WaveSeed(1,2), '% 10.0f'),nLines_HDipt);% 2nd random waveseed
changeline(HDipt,13,num2str(WaveHs(1), '% 10.2f'),nLines_HDipt);% WaveHs
changeline(HDipt,14,num2str(WaveTp(1), '% 10.6f'),nLines_HDipt);% WaveTp

% TurbSim
!.\Wind\Turbsim.exe .\Wind\Turbsim13pt2MW.inp
...
name=[Parentfoler,'\OffShr_Results\', 'SNL_13pt2MW_OffShr_',model,'Uref_',num2str(WindVel(1),'%10.2f'),'_',num2str(WaveHs(1),'%10.2f'),'.mat'];
